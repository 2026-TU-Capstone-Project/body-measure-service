...
class BlazePose():
    ...
    def build_model(self, model_type):
        ...
        if self.attention_model is not None :
            x = tf.keras.layers.Concatenate(axis=3)([x, self.attention_model.outputs[0]])
        # In: 1, 32, 32, 96
        x = self.conv13a(x) + self.conv13b(y3)
        if self.attention_model is not None :
            x = tf.keras.layers.Concatenate(axis=3)([x, self.attention_model.outputs[1]])
        # In: 1, 16, 16, 192
        x = self.conv14a(x) + self.conv14b(y4)
        if self.attention_model is not None :
            x = tf.keras.layers.Concatenate(axis=3)([x, self.attention_model.outputs[2]])
        # In: 1, 8, 8, 288
        x = self.conv15(x)
        # In: 1, 2, 2, 288
        joints = self.conv16(x)
        output = tf.keras.layers.Reshape((self.num_keypoints,))(joints)
